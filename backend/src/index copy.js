...
// 导入您的通用 API 上下文工具
import apiContext from './utils/apiContext'; // <--- 新增：导入 apiContext

// 导入您的认证和用户管理路由
import { authRoutes } from './routes/auth-api'; // <--- 保持：导入 authRoutes
...
import { dashboardRouters } from './routes/dashboard-api'; 
...
// 注册 authRoutes 中的所有路由
authRoutes.forEach(route => {
    apiRouter[route.method.toLowerCase()](route.pattern, async (request, env, ctx) => {
        // 在每次请求处理前，确保 apiContext 中的 Supabase 客户端是最新的
        // 尽管它应该在 initializeSupabaseClient 中设置一次，但这种方式更健壮
        apiContext.setSupabaseClient(supabaseClient);

        // 调用原始的路由处理器，并传入 apiContext
        return route.handler(request, env, apiContext);
    });
});
// 注册 incomeRoutes 中的所有路由
incomeRoutes.forEach(route => {
    apiRouter[route.method.toLowerCase()](route.pattern, async (request, env, ctx) => {
        apiContext.setSupabaseClient(supabaseClient); // 确保 apiContext 中的 Supabase 客户端是最新的
        return route.handler(request, env, apiContext);
    });
});
...
booknoteRoutes.forEach(route => {
    apiRouter[route.method.toLowerCase()](route.pattern, async (request, env, ctx) => {
        apiContext.setSupabaseClient(supabaseClient); // 确保 apiContext 中的 Supabase 客户端是最新的
        return route.handler(request, env, apiContext);
    });
});
diaryRoutes.forEach(route => {
    apiRouter[route.method.toLowerCase()](route.pattern, async (request, env, ctx) => {
        apiContext.setSupabaseClient(supabaseClient); // 确保 apiContext 中的 Supabase 客户端是最新的
        return route.handler(request, env, apiContext);
    });
});
dashboardRouters.forEach(route => {
    apiRouter[route.method.toLowerCase()](route.pattern, async (request, env, ctx) => {
        apiContext.setSupabaseClient(supabaseClient); // 确保 apiContext 中的 Supabase 客户端是最新的
        return route.handler(request, env, apiContext);
    });
...
//apiRouter.all('*', () => new Response('API 路由未找到', { status: 404 }));
apiRouter.all('*', (request, env, ctx) => { // 接收 request, env, ctx 便于调试或记录
    ...
    async fetch(request, env, ctx) {
        ...
        if (!isSupabaseClientInitialized) {
            try {
                ...
                //return new Response(`服务不可用: 初始化后端服务失败。${initError.message}`, { status: 503 });
                return new Response(JSON.stringify({
                    message: `服务不可用: 初始化后端服务失败。${initError.message}`,
                    details: initError.stack // 提供更多调试信息
                }), {
                    status: 503,
                    headers: { 'Content-Type': 'application/json' }
                });
        ...
        // --- 2. 让路由器处理传入请求。 ---
        // 传递 request, env, ctx 给 itty-router 的 handle 方法
        //return apiRouter.handle(request, env, ctx);
        return apiRouter.handle(request, env, ctx).catch(async (error) => {
