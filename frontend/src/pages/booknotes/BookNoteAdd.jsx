...
// For example: If BookNoteAdd.jsx is in 'src/pages/', and MainLayout.jsx is in 'src/',
...
import { BookText, User, Calendar, Tag, Star, PlusCircle, XCircle, CheckCircle, Slash } from 'lucide-react'; // Icons
...
const BookNoteAdd = () => {
  ...
  const [readDate, setReadDate] = useState(new Date().toISOString().substring(0, 10)); // Default to current date
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/booknotes', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` // Send JWT for authentication
        },
        body: JSON.stringify(bookNoteData),
      });

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.message || '添加读书笔记失败。');
      }

      setMessage({ type: 'success', text: data.message || '读书笔记添加成功！' });
      ...
      setCategory('');
      setTags('');
      setReadDate(new Date().toISOString().substring(0, 10));
      setRating('');
      setNotes('');
      ...
      console.error('Add book note error:', err);
      setMessage({ type: 'error', text: err.message || '添加读书笔记失败，请稍后再试。' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <MainLayout pageTitle="新增读书笔记"> {/* Wrap with MainLayout */}
        ...
        <Card className="w-full max-w-3xl shadow-xl rounded-xl overflow-hidden">
          <CardHeader className="bg-blue-600 text-white text-center py-5">
            <CardTitle className="text-2xl font-bold flex items-center justify-center">
              <BookText className="mr-2 h-7 w-7" />新增读书笔记
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div className="col-span-full flex flex-col sm:flex-row justify-end gap-3">
                <Button type="submit" className="flex-grow sm:flex-none bg-blue-600 hover:bg-blue-700 text-white rounded-md shadow-md py-2.5 text-base" disabled={loading}>
                  <PlusCircle className="mr-2 h-5 w-5" />{loading ? '添加中...' : '添加笔记'}
                </Button>
