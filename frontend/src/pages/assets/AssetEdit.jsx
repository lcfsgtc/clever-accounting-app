...
import { Package, DollarSign, Calendar, Info, TrendingUp, ClipboardPen, Percent, Save, ArrowLeft, CheckCircle, XCircle, Edit } from 'lucide-react'; // Icons for form fields and actions
...
const AssetEdit = ({ assetId: propAssetId }) => {
  const [asset, setAsset] = useState(null); // To store the original fetched data
  ...
  const [depreciationMethod, setDepreciationMethod] = useState('直线折旧');
  ...
  const fetchAsset = useCallback(async () => {
    ...
    try {
      ...
      setAsset(data.asset); // Store full asset data
      setName(data.asset.name);
      setType(data.asset.type);
      setQuantity(data.asset.quantity);
      setCost(data.asset.cost);
      setCurrentValue(data.asset.currentValue);
      setPurchaseDate(formatDateForInput(data.asset.purchaseDate)); // Format date for input[type="date"]
      setCondition(data.asset.condition);
      setDepreciationMethod(data.asset.depreciationMethod);
      setDepreciationRate(data.asset.depreciationRate);
      setNotes(data.asset.notes);

    } catch (err) {
      console.error('Error fetching asset:', err);
      setMessage({ type: 'error', text: `加载资产数据失败: ${err.message}` });
    } finally {
      setLoading(false);
    }
  }, [currentAssetId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: data.message || '资产更新成功！' });
      ...
      setMessage({ type: 'error', text: err.message || '更新资产失败，请稍后再试。' });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!asset) {
    return (
      <MainLayout pageTitle="编辑资产">
        <div className="container mx-auto p-4 text-center text-red-500">资产未找到或无权访问。</div>
      </MainLayout>
    );
  }

  return (
    <MainLayout pageTitle="编辑资产"> {/* 使用 MainLayout 包装组件 */}
        ...
        <Card className="w-full max-w-2xl shadow-xl rounded-xl overflow-hidden">
          <CardHeader className="bg-blue-600 text-white text-center py-5">
            <CardTitle className="text-2xl font-bold flex items-center justify-center">
              <Edit className="mr-2 h-7 w-7" />编辑资产
            </CardTitle>
          </CardHeader>
